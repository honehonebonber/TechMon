...
class BattleViewController: UIViewController {
    ...
    var enemy: Enemy = Enemy(name:"ドラゴン1",imageName: "m87.png")
    var player: Player = Player(name:"勇者",imageName:"m19.png")
    ...
    func finishBattle(vanishImageView: UIImageView,winPlayer: Bool){
        ...
        if winPlayer == true {
            ...
            keikenti = keikenti + 10
            // NSUserDefaultsに保存
            NSUserDefaults.standardUserDefaults().setInteger(mahouseki, forKey: "mahouseki")
            NSUserDefaults.standardUserDefaults().synchronize()
            
            NSUserDefaults.standardUserDefaults().setFloat(keikenti, forKey: "keikenti")
            NSUserDefaults.standardUserDefaults().synchronize()
            NSLog("魔法石の数は...%dです", mahouseki)
        }else{
